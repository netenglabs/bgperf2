...
| `plateau_start_s` | seconds or null | When the export count last changed. The target stopped exporting here; every later poll is evidence it stayed stopped. **Dated to the witness read, not to the monitor poll that carried it** — the target's gauge is read on its own loop, so a reading reaches a monitor sample up to `witness_age_s` later (signed, so sometimes earlier). Using the monitor's stamp put both ends of `monitor_lag_s` on one clock and collapsed it to exactly 0.0. `null` for an undated reading, which no resolved section can contain. |
...
| `monitor_lag_s` | seconds or null | `complete_s - plateau_start_s`: how long after the target finished the monitor took to hold it. The part of the tail this series can see on its own. **Signed, and nothing clamps it** — the two ends are independent poll loops, so the monitor can be seen holding the table before the target is next read showing it finished, and 1019 of 2892 recorded samples carry a negative `witness_age_s`. The rule `post_injection_tail_s` and `export.monitor_delta_s` already follow. Never read it without the next row. |
| `monitor_lag_resolution_s` | seconds or null | The wider of the two looks bounding `monitor_lag_s`: the monitor's gap at completion, and the witness read gap at the plateau start. A magnitude at or under this is the two instruments agreeing **within their resolution** and must not be read as a duration — the rule `first_prefix_s` and `injection_s` are qualified by. All 27 recorded MRT rows of the 64 GB campaign are inside their own bound. |
